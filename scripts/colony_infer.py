...
Usage:
  .venv311/bin/python scripts/colony_infer.py --config config/experiments/qwen3_endogenous.yaml \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--members", type=str, default="auto", help="comma-separated organelle IDs or 'auto'")
...
def main() -> None:
    ...
    if args.members.strip().lower() == "auto":
        for _ in range(2):
            ...
            population.register(Genome(organelle_id=oid, drive_weights={"novelty": 0.4}, gate_bias=0.0, rank=cfg.host.max_lora_rank))
        members: List[str] = host.list_organelle_ids()[:2]
    else:
        members = [m.strip() for m in args.members.split(",") if m.strip()]
    env = GridEnvironment(cfg.grid, cfg.controller, cfg.pricing, cfg.canary, seed=123, reward_bonus=cfg.environment.success_reward_bonus, failure_cost_multiplier=cfg.environment.failure_cost_multiplier, lp_alpha=getattr(cfg.curriculum, "lp_alpha", 0.5))
