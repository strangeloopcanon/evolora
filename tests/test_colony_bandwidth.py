...
def test_colony_bandwidth_limits_reads(tmp_path):
    ...
    pop.register(
        Genome(
            organelle_id=oid,
            drive_weights={"novelty": 0.0},
            gate_bias=0.0,
            rank=cfg.host.max_lora_rank,
        )
    )
    assim = AssimilationTester(
        cfg.evolution.assimilation_threshold, cfg.evolution.assimilation_p_value, 0
    )
...
def test_colony_bandwidth_hazard_scaling():
    ...
    oid = host.spawn_organelle(rank=cfg.host.max_lora_rank)
    pop.register(
        Genome(organelle_id=oid, drive_weights={}, gate_bias=0.0, rank=cfg.host.max_lora_rank)
    )
    assim = AssimilationTester(
        cfg.evolution.assimilation_threshold, cfg.evolution.assimilation_p_value, 0
    )
    ...
    expected_budget = (
        min(
            cfg.assimilation_tuning.colony_bandwidth_base,
            5.0 * cfg.assimilation_tuning.colony_bandwidth_frac,
        )
        * cfg.assimilation_tuning.colony_hazard_bandwidth_scale
    )
    assert abs(meta.get("bandwidth_budget", 0.0) - expected_budget) < 1e-6
