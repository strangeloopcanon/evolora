...
def test_team_accept_gate_true_and_false():
    ...
    assert (
        EcologyLoop._team_accept(ci_low=0.12, baseline=0.08, margin=0.01, n=10, min_tasks=8) is True
    ...
    assert (
        EcologyLoop._team_accept(ci_low=0.10, baseline=0.095, margin=0.01, n=12, min_tasks=8)
        is False
    )  # type: ignore[attr-defined]
...
def test_team_power_proxy_blocks_promotion_when_low():
    ...
    loop = EcologyLoop(
        ...
        environment=SimpleNamespace(rng=__import__("random").Random(0)),
        population=Pop(),
