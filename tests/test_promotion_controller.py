...
def test_promotion_controller_adjusts_thresholds():
    ...
    loop = EcologyLoop(
        ...
        environment=SimpleNamespace(
            ...
            sample_task=lambda: SimpleNamespace(
                prompt="p", price=1.0, evaluate=lambda a: (True, SimpleNamespace(total=1.0))
            ),
        ),
    ...
    if step > 0.0 and target > 0.0 and err > 0:
        cfg.assimilation_tuning.team_holdout_margin = max(
            cfg.assimilation_tuning.team_margin_min,
            cfg.assimilation_tuning.team_holdout_margin - step,
        )
        cfg.assimilation_tuning.team_min_power = max(
            cfg.assimilation_tuning.team_power_min, cfg.assimilation_tuning.team_min_power - step
        )
