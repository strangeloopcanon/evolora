...
def test_sample_task_lp_with_canary_path() -> None:
    ...
    env = SimpleNamespace(
        ...
        sample_task_from_cell=lambda cell, canary=False: SimpleNamespace(prompt="ok", price=1.0, evaluate=lambda a: (True, SimpleNamespace(total=1.0))),
        sample_task=lambda: SimpleNamespace(prompt="nope", price=1.0, evaluate=lambda a: (True, SimpleNamespace(total=1.0))),
    )
    loop = EcologyLoop(config=cfg, host=SimpleNamespace(), environment=env, population=SimpleNamespace(), assimilation=SimpleNamespace())
...
def test_sample_task_with_policy_bias() -> None:
    ...
    env = SimpleNamespace(
        controller=SimpleNamespace(),
        sample_task_from_cell=lambda cell, canary=False: SimpleNamespace(prompt=f"{cell}", price=1.0, evaluate=lambda a: (True, SimpleNamespace(total=1.0))),
        sample_task=lambda: SimpleNamespace(prompt="fallback", price=1.0, evaluate=lambda a: (True, SimpleNamespace(total=1.0))),
        rng=SimpleNamespace(random=lambda: 0.0),
    )
    loop = EcologyLoop(config=cfg, host=SimpleNamespace(), environment=env, population=SimpleNamespace(), assimilation=SimpleNamespace())
