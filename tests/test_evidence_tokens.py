...
def test_apply_evidence_tokens_reduces_window():
    ...
    adjusted, used = loop._apply_evidence_tokens("org", available_even=6, min_window=10, min_samples_required=2)
    assert used is True
    assert adjusted <= 6
    # No tokens left -> no change
    loop.population.tokens = 0
    adjusted_again, used_again = loop._apply_evidence_tokens("org", available_even=4, min_window=8, min_samples_required=2)
...
def test_apply_evidence_tokens_uses_override_floor():
    ...
    loop._power_econ_stats = {}
    adjusted, used = loop._apply_evidence_tokens("org", available_even=2, min_window=12, min_samples_required=2)
    assert used is True
    assert adjusted == 2
