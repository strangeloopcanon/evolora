...
def test_run_team_episode_best_of_two_and_records(monkeypatch) -> None:
    ...
    def step(prompt: str, intent: str, max_routes: int, allowed_organelle_ids):  # noqa: ARG002
        ...
        return SimpleNamespace(envelope=env, routes=[SimpleNamespace(organelle_id=oid)], responses={oid: DummyMetrics()}, latency_ms=0.1)
    ...
    loop._record_episode = lambda task, oid, reward, metrics, settlement, success, utilisation: recorded.append((oid, success))  # type: ignore[method-assign]
...
def test_select_synergy_pair_fallback_by_roi() -> None:
    ...
    loop = EcologyLoop(config=cfg, host=SimpleNamespace(), environment=SimpleNamespace(), population=Pop(), assimilation=SimpleNamespace())
    pair = loop._select_synergy_pair()
