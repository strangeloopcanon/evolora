...
# Running example: timestamp regex from docs
TIMESTAMP_INTENT = """Match a valid log timestamp of the form YYYY-MM-DD HH:MM:SS with:
- year: 2000–2099
- month: 01–12
- day: 01–31
...
TIMESTAMP_NON_MATCHES = [
    "2023-01-01 24:00:00",  # Invalid hour
    "2023-13-01 12:00:00",  # Invalid month
    "2023-01-32 12:00:00",  # Invalid day
    "1999-01-01 12:00:00",  # Year out of range
    "2023-1-01 12:00:00",  # Missing leading zero
    "2023-01-01  12:00:00",  # Double space
    " 2023-01-01 12:00:00",  # Leading space
]


def generate_timestamp_tasks() -> list[RegexTask]:
    """Generate comprehensive tasks for the timestamp running example."""
    tasks = []

    # === Recognition Tasks ===
    recognition_cases = [
        ("2023-01-01 24:00:00", False, "Invalid hour 24"),
        ("2023-12-31 23:59:59", True, "Valid timestamp"),
        ("2023-00-15 12:00:00", False, "Invalid month 00"),
        ("2023-01-01 12:60:00", False, "Invalid minute 60"),
    ]

    for string, should_match, reason in recognition_cases:
        expected = "yes" if should_match else "no"
        tasks.append(
            RegexTask(
                task_id=f"recog_{string.replace(' ', '_').replace(':', '')}",
                prompt=f'Given this regex for matching timestamps:\n{TIMESTAMP_REGEX}\n\nDoes it match "{string}"? Answer yes or no, and briefly explain why.',
                capability=CapabilityAxis.RECOGNITION,
                target_regex=TIMESTAMP_REGEX,
                expected_answer=expected,
                metadata={"reason": reason, "test_string": string},
            )
        )

    # === Synthesis Tasks ===
    test_cases = [RegexTestCase(s, True) for s in TIMESTAMP_MATCHES] + [
        RegexTestCase(s, False) for s in TIMESTAMP_NON_MATCHES
    ]

    # Basic synthesis
    tasks.append(
        RegexTask(
            task_id="synth_timestamp_basic",
            prompt=f"{TIMESTAMP_INTENT}\n\nWrite a regex pattern that matches valid timestamps. Respond with only the regex.",
    ...
    buggy_patterns = [
        (
            r"^(20\d{2})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]) (?:[01]\d|2[0-4]):[0-5]\d:[0-5]\d$",
            "Allows hour 24",
        ),
        (
            r"^(20\d{2})-(\d{2})-(\d{2}) (?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d$",
            "Allows invalid months/days like 00 or 99",
        ),
        (
            r"^(20\d{2})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d$",
    ...
    for i, (buggy, description) in enumerate(buggy_patterns):
        tasks.append(
            RegexTask(
                task_id=f"debug_timestamp_{i}",
                prompt=f"This regex is supposed to match valid timestamps ({TIMESTAMP_INTENT}) but has a bug:\n{buggy}\n\nFix the regex. Respond with only the corrected pattern.",
                capability=CapabilityAxis.DEBUGGING,
                target_regex=buggy,
                test_cases=test_cases,
                metadata={"bug_description": description},
            )
        )

    # === Refactoring Tasks ===
    verbose_pattern = r"^(20[0-9][0-9])-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1]) ([0-1][0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$"
    tasks.append(
        RegexTask(
            task_id="refactor_timestamp",
            prompt=f"Simplify this regex without changing its matching behavior:\n{verbose_pattern}\n\nRespond with only the simplified pattern.",
            capability=CapabilityAxis.REFACTORING,
            target_regex=verbose_pattern,
            test_cases=test_cases,
        )
    )
...
def generate_holdout_tasks() -> list[RegexTask]:
    """Generate tasks testing different hold-out structures."""
    tasks = []

    # === Operator Hold-Out ===
    # Train: regexes without alternation, Test: requires alternation
    tasks.append(
        RegexTask(
            task_id="holdout_operator_alternation",
            prompt="Write a regex to match either 'cat' or 'dog' as a complete word. Respond with only the regex.",
            capability=CapabilityAxis.SYNTHESIS,
            holdout_type=HoldOutType.OPERATOR,
            test_cases=[
                RegexTestCase("cat", True),
                RegexTestCase("dog", True),
                RegexTestCase("cats", False),
                RegexTestCase("hotdog", False),
                RegexTestCase("catalog", False),
            ],
            metadata={"held_out_concept": "alternation (|)"},
        )
    )

    # === Composition Hold-Out ===
    # Train: operators separately, Test: combined
    tasks.append(
        RegexTask(
            task_id="holdout_composition_mixed",
            prompt="Write a regex to match day numbers 01-31 where days 1-9 must have leading zero. Respond with only the regex.",
            capability=CapabilityAxis.SYNTHESIS,
            holdout_type=HoldOutType.COMPOSITION,
            test_cases=[
                RegexTestCase("01", True),
                RegexTestCase("15", True),
                RegexTestCase("31", True),
                RegexTestCase("00", False),
                RegexTestCase("32", False),
                RegexTestCase("1", False),
                RegexTestCase("9", False),
            ],
            metadata={"held_out_concept": "alternation + character classes combined"},
        )
    )

    # === Semantic Coupling Hold-Out ===
    # Train: date-only and time-only, Test: combined constraints
    tasks.append(
        RegexTask(
            task_id="holdout_semantic_datetime",
            prompt="Write a regex to match datetime strings in ISO 8601 format: YYYY-MM-DDTHH:MM:SS. Years should be 1900-2099, months 01-12, days 01-31, hours 00-23, minutes and seconds 00-59. Respond with only the regex.",
            capability=CapabilityAxis.SYNTHESIS,
            holdout_type=HoldOutType.SEMANTIC_COUPLING,
            test_cases=[
                RegexTestCase("2023-06-15T14:30:00", True),
                RegexTestCase("1999-12-31T23:59:59", True),
                RegexTestCase("2023-13-01T12:00:00", False),
                RegexTestCase("2023-06-15T25:00:00", False),
                RegexTestCase("2023-06-15 14:30:00", False),  # Space instead of T
            ],
            metadata={"held_out_concept": "joint date + time constraints"},
        )
    )

    # === Surface-Form Hold-Out ===
    # Train: YYYY/MM/DD, Test: YYYY-MM-DD
    tasks.append(
        RegexTask(
            task_id="holdout_surface_delimiter",
            prompt="Write a regex to match dates in format YYYY.MM.DD (using dots as separators). Year 2000-2099, month 01-12, day 01-31. Respond with only the regex.",
            capability=CapabilityAxis.SYNTHESIS,
            holdout_type=HoldOutType.SURFACE_FORM,
            test_cases=[
                RegexTestCase("2023.06.15", True),
                RegexTestCase("2000.01.01", True),
                RegexTestCase("2023-06-15", False),  # Wrong delimiter
                RegexTestCase("2023/06/15", False),  # Wrong delimiter
                RegexTestCase("2023.13.01", False),  # Invalid month
            ],
            metadata={"held_out_concept": "delimiter variation (dot vs dash)"},
        )
    )
...
def generate_mutation_tasks() -> list[RegexTask]:
    """Generate mutation/counterfactual tasks."""
    tasks = []

    # === Effect Prediction Tasks ===
    mutations = [
        {
            "original": r"^(?:[01]\d|2[0-3]):[0-5]\d$",
            "mutated": r"^(?:[01]\d|2\d):[0-5]\d$",
            "mutation_type": MutationType.WIDEN_RANGE,
            "question": "The hour part was changed from '2[0-3]' to '2\\d'. What new strings will now match that didn't before?",
            "answer_keywords": ["24", "25", "26", "27", "28", "29"],
        },
        {
            "original": r"^(?:0[1-9]|1[0-2])$",
            "mutated": r"^0[1-9]|1[0-2]$",
            "mutation_type": MutationType.REMOVE_GROUPING,
            "question": "The non-capturing group was removed. How does the matching behavior change?",
            "answer_keywords": ["anchor", "beginning", "end", "partial", "alternation"],
        },
        {
            "original": r"^\d{4}$",
            "mutated": r"^\d+$",
            "question": "The quantifier was changed from {4} to +. What new strings will now match?",
            "mutation_type": MutationType.CHANGE_QUANTIFIER,
            "answer_keywords": ["1", "12", "123", "12345", "any", "length"],
        },
    ]
    ...
    for i, mut in enumerate(mutations):
        tasks.append(
            RegexTask(
                task_id=f"mutation_effect_{i}",
                prompt=f"Original regex: {mut['original']}\nMutated regex: {mut['mutated']}\n\n{mut['question']}",
                capability=CapabilityAxis.RECOGNITION,  # Effect prediction is a form of recognition
                mutation_type=mut["mutation_type"],
                target_regex=mut["original"],
                metadata={
                    "mutated_regex": mut["mutated"],
                    "required_keywords": mut["answer_keywords"],
                },
            )
        )
    ...
    repair_cases = [
        {
            "original": r"^(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d$",
            "broken": r"^(?:[01]\d|2\d):[0-5]\d:[0-5]\d$",
            "mutation_type": MutationType.WIDEN_RANGE,
            "issue": "allows hours 24-29",
            "test_cases": [
                RegexTestCase("12:30:45", True),
                RegexTestCase("23:59:59", True),
                RegexTestCase("24:00:00", False),
                RegexTestCase("29:30:00", False),
            ],
        },
        {
            "original": r"^\d{3}-\d{4}$",
            "broken": r"\d{3}-\d{4}",
            "mutation_type": MutationType.REMOVE_ANCHOR,
            "issue": "matches partial strings",
            "test_cases": [
                RegexTestCase("123-4567", True),
                RegexTestCase("abc123-4567xyz", False),
                RegexTestCase("123-4567890", False),
            ],
        },
    ]

    for i, rc in enumerate(repair_cases):
        tasks.append(
            RegexTask(
                task_id=f"mutation_repair_{i}",
                prompt=f"This regex was mutated and now {rc['issue']}:\n{rc['broken']}\n\nFix it to restore the original behavior. Respond with only the corrected regex.",
                capability=CapabilityAxis.DEBUGGING,
                mutation_type=rc["mutation_type"],
                target_regex=rc["original"],
                test_cases=rc["test_cases"],
                metadata={"broken_regex": rc["broken"], "issue": rc["issue"]},
            )
        )
...
def generate_full_eval_suite() -> list[RegexTask]:
    """Generate the complete evaluation suite."""
    tasks = []
    tasks.extend(generate_timestamp_tasks())
    tasks.extend(generate_holdout_tasks())
    tasks.extend(generate_mutation_tasks())
    return tasks


def save_tasks_to_jsonl(tasks: list[RegexTask], path: Path) -> None:
